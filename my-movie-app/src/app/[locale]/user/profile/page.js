...
const ProfilePage = async ({ params: { locale } }) => {
  const { user: userSession } = await getServerSession(); // Permet de récupérer la session d'un utilisateur dans un composant serveur

  const { movieLikes } = await prisma.user.findFirst({
    where: { email: userSession.email },
    include: { // Pour inclure les relations de la BDD dans le résultat de l'appel
      movieLikes: true,
    },
  });
